((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,C,D,B={
cWR(d,e,f,g,h){return B.jru(d,e,f,g,h)},
jru(d,e,f,g,h){var x=0,w=A.l(y.e),v,u,t,s,r,q,p,o
var $async$cWR=A.h(function(i,j){if(i===1)return A.i(j,w)
for(;;)switch(x){case 0:p={}
o=$.eg().a.h(0,h)
o.toString
if(f==null){u=o.e
u===$&&A.a()
t=u}else t=f
if(e==null){o=o.d
o===$&&A.a()
s=o}else s=e
x=3
return A.d(B.hTN(d,g),$async$cWR)
case 3:if(j!==!0){v=!1
x=1
break}r=J.N(t.a,s).c
r.M(0,g.b)
p.a=0
q=A.o(y.w,y.q)
r.u(0,new B.hHX(p,q))
J.N(t.a,s).c=q
t.t()
v=!0
x=1
break
case 1:return A.j(v,w)}})
return A.k($async$cWR,w)},
hHX:function hHX(d,e){this.a=d
this.b=e},
hTN(d,e){var x=0,w=A.l(y.h),v,u
var $async$hTN=A.h(function(f,g){if(f===1)return A.i(g,w)
for(;;)switch(x){case 0:if(e==null){v=null
x=1
break}u=A.e("deleting",null,null,!0)
x=3
return A.d(A.hw(null,A.bC(A.e("doDeleteTreatment",null,null,!0)+'  - "'+e.z+'" ?',null,null,null,null,null,null,null,C.C,null,null,null,null),d,null,new B.hTO(),new B.hTP(),!0,u),$async$hTN)
case 3:v=g
x=1
break
case 1:return A.j(v,w)}})
return A.k($async$hTN,w)},
hTO:function hTO(){},
hTP:function hTP(){},
aCL(d,e,f,g,h,i,j){var x=0,w=A.l(y.b),v
var $async$aCL=A.h(function(k,l){if(k===1)return A.i(l,w)
for(;;)switch(x){case 0:A.aF("addOrUpdateTreatment")
x=3
return A.d(A.jd(d,null,D.iUY(e,f,g,h,i,j)),$async$aCL)
case 3:v=l
x=1
break
case 1:return A.j(v,w)}})
return A.k($async$aCL,w)}}
J=c[1]
A=c[0]
C=c[2]
D=c[318]
B=a.updateHolder(c[415],B)
var z=a.updateTypes([])
B.hHX.prototype={
$2(d,e){var x=this.a,w=x.a
e.b=C.i.m(w)
this.b.i(0,C.i.m(w),e);++x.a},
$S:16}
B.hTO.prototype={
$1(d){A.Q(d,!1).I(!1)
return null},
$S:5}
B.hTP.prototype={
$1(d){return this.bXI(d)},
bXI(d){var x=0,w=A.l(y.v)
var $async$$1=A.h(function(e,f){if(e===1)return A.i(f,w)
for(;;)switch(x){case 0:A.Q(d,!1).I(!0)
return A.j(null,w)}})
return A.k($async$$1,w)},
$S:80};(function inheritance(){var x=a.inherit,w=a.inheritMany
x(B.hHX,A.bZ)
w(A.aO,[B.hTO,B.hTP])})()
var y={w:A.t("n"),q:A.t("eq"),e:A.t("J"),b:A.t("@"),h:A.t("J?"),v:A.t("~")}};
(a=>{a["08+IS0slmwvW5Ez7fr6kiIjULNQ="]=a.current})($__dart_deferred_initializers__);